...
export default function EditEmailCampaignForm({ campaign, templates, profiles }: {
  campaign:  Campaign;
  templates: Template[];
  profiles:  Profile[];
}) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await updateEmailCampaign(campaign.id, fd);
    setLoading(false);
    if (res.error) setError(res.error);
    else { setSaved(true); setTimeout(() => router.push('/admin/notificaciones/email'), 800); }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-3">
        ...
        <a href="/admin/notificaciones/email"
