...
import { updatePsychologyCase } from './actions';
...
export default function EditCaseForm({ caseId, summary, athleteName, profileName, openedAt }: Props) {
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await updatePsychologyCase(caseId, formData);
      if (result.error) {
      ...
      <div>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Psicólogo/a: {profileName}</p>
          </div>
