...
import { updateMedicalCase } from './actions';
...
export default function EditCaseForm({ caseId, condition, notes, athleteName, profileName, openedAt }: Props) {
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await updateMedicalCase(caseId, formData);
      if (result.error) {
  ...
  if (!editing) {
    return (
      <div>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {profileName}{condition ? ` · ${condition}` : ''}
            </p>
          </div>
        ...
        {notes && (
          <p className="mt-3 text-sm text-gray-600 italic border-l-2 border-gray-200 pl-3">{notes}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3">
      ...
      <form ref={formRef} action={handleSubmit} className="space-y-2">
        <div>
          <label className="block text-xs font-medium mb-0.5">Condición / Diagnóstico</label>
          <input
            name="condition"
            type="text"
        ...
        <div>
          <label className="block text-xs font-medium mb-0.5">Notas del caso</label>
          <textarea
            name="notes"
            rows={3}
            defaultValue={notes ?? ''}
