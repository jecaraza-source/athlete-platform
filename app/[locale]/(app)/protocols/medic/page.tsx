...
export default async function MedicProtocolsPage() {
  ...
  const protocol  = await getProtocolByDiscipline('medic');
  ...
  return (
    <main className="p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mt-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-rose-700">Protocolo: Médico</h1>
          {protocol && (
        ...
        {signedUrl && (
          <a href={signedUrl} download className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700 hover:bg-rose-100 transition-colors">
            ⤓ Descargar PDF
          </a>
        )}
      </div>
