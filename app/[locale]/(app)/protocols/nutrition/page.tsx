...
export default async function NutritionProtocolsPage() {
  await requireAuthenticated();
  const protocol  = await getProtocolByDiscipline('nutrition');
  ...
  return (
    <main className="p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mt-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-emerald-700">Protocolo: Nutrición</h1>
          {protocol && (
        ...
        {signedUrl && (
          <a href={signedUrl} download className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-700 hover:bg-emerald-100 transition-colors">
            ⤓ Descargar PDF
          </a>
        )}
      </div>
