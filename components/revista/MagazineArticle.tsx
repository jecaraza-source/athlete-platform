...
export function MagazineArticle({ article, appUrl, showShare = true }: MagazineArticleProps) {
  ...
  // Dividir la narrativa en párrafos para intercalar fotos
  const paragraphs = narrative.narrative_text.split(/\n+/).filter(Boolean);

  // Índices donde insertar fotos: después del párrafo 1 y párrafo 3
  const photoInsertPoints = new Set([1, 3]);

  return (
    <article className="max-w-2xl mx-auto print:max-w-none">
      {/* Hero */}
      {coverPhoto && (
        <div className="relative w-full aspect-[16/9] rounded-2xl overflow-hidden mb-6 print:rounded-none">
          <Image
      ...
      {/* Narrativa con fotos intercaladas */}
      <div className="prose prose-gray prose-sm sm:prose-base max-w-none">
        {paragraphs.map((paragraph, i) => (
          <div key={i}>
            ...
            {/* Foto intercalada */}
            {photoInsertPoints.has(i) && otherPhotos[Math.floor(i / 2)] && (
              <figure className="my-6 print:break-inside-avoid">
                <div className="relative w-full aspect-[4/3] rounded-xl overflow-hidden print:rounded-none">
                  <Image
                    src={getThumbnailUrl(otherPhotos[Math.floor(i / 2)].storage_path)}
                    alt={otherPhotos[Math.floor(i / 2)].alt_text}
                    fill
                    sizes="(max-width: 768px) 100vw, 600px"
                    className="object-cover"
                  />
                </div>
                {otherPhotos[Math.floor(i / 2)].caption && (
